...
GeneConvert <- function(
  ...
  if (is.null(biomart)) {
    ...
    biomart <- match.arg(
      biomart,
      choices = c("ensembl", "protists_mart", "fungi_mart", "plants_mart"),
      several.ok = TRUE
    )
    url <- setNames(
      object = c(
...
searchDatasets <- function(datasets, pattern) {
  colIdx <- vapply(
    datasets,
    FUN = function(x) grepl(pattern = pattern, x = x, ignore.case = TRUE),
...
CellScoring <- function(
  ...
  expressed <- names(which(
    rowSums(GetAssayData(srt, layer = layer, assay = assay) > 0) > 0
  ))
  features <- lapply(setNames(names(features), names(features)), function(x) {
  ...
  for (i in seq_along(split_list)) {
    ...
    if (method == "Seurat") {
      ...
      CellRank <- AUCell::AUCell_buildRankings(
        as_matrix(GetAssayData(srt_sp, layer = layer, assay = assay)),
        BPPARAM = BPPARAM,
        plotStats = FALSE
      )
      ...
      features_keep <- features[!names(features) %in% filtered]
      features_nm <- features_raw[!names(features) %in% filtered]
      scores <- as.data.frame(t(AUCell::getAUC(cells_AUC)))[, names(
        features_keep
      )]
      colnames(scores) <- make.names(paste(name, names(features_nm), sep = "_"))
    }
  ...
  scores_mat <- do.call(
    rbind,
    lapply(scores_list, function(x) {
      x[intersect(rownames(x), colnames(srt)), features_used, drop = FALSE]
    })
  )

  if (isTRUE(new_assay)) {
    srt[[name]] <- CreateAssayObject(
      counts = t(as_matrix(scores_mat[colnames(srt), , drop = FALSE]))
    )
    srt[[name]] <- AddMetaData(
      object = srt[[name]],
      metadata = data.frame(termnames = features_nm_used[colnames(scores_mat)])
    )
  } else {
...
metap <- function(
  p,
  method = c("maximump", "minimump", "wilkinsonp", "meanp", "sump", "votep"),
  ...
) {
...
FindConservedMarkers2 <- function(
  ...
  if (is.null(cells.1)) {
    ...
    for (i in 1:num.groups) {
      ...
      if (is.null(x = ident.2)) {
        ...
        ident.use.2 <- names(
          x = which(x = table(Idents(object = object)[cells.2.use]) > 0)
        )
        ident.2 <- gsub(
  ...
  genes.conserved <- Reduce(
    f = intersect,
    x = lapply(X = marker.test, FUN = function(x) {
      return(rownames(x = x))
    })
  )
  ...
  logFC.codes <- colnames(x = markers.combined)[grepl(
    pattern = "*avg_log.*FC$",
  ...
  pval.codes <- colnames(x = markers.combined)[grepl(
    pattern = "*_p_val$",
    x = colnames(x = markers.combined)
  )]
...
#'   layer = "data", aggregate_fun = function(x) mean(expm1(x)) + 1,
#'   features = DisturbedMarkers_specific$gene, feature_split = DisturbedMarkers_specific$group1,
...
RunDEtest <- function(
  ...
  if (!is.null(cells1) || !is.null(group1)) {
    ...
    if (markers_type == "all") {
      ...
      if (!is.null(markers) && nrow(markers) > 0) {
        ...
        if ("p_val" %in% colnames(markers)) {
          markers[, "p_val_adj"] <- p.adjust(
            markers[, "p_val"],
            method = p.adjust.method
          )
        }
        markers[, "test_group_number"] <- as.integer(table(markers[[
          "gene"
        ]])[markers[, "gene"]])
        MarkersMatrix <- as.data.frame.matrix(table(markers[, c(
          "gene",
          "group1"
        )]))
    ...
    cell_group <- setNames(
      unlist(lapply(cell_group, function(x) x), use.names = FALSE),
    ...
    if (markers_type == "all") {
      ...
      if (!is.null(AllMarkers) && nrow(AllMarkers) > 0) {
        ...
        AllMarkers[, "test_group_number"] <- as.integer(table(AllMarkers[[
          "gene"
        ]])[AllMarkers[, "gene"]])
        AllMarkersMatrix <- as.data.frame.matrix(table(AllMarkers[, c(
          "gene",
          "group1"
        )]))
    ...
    if (markers_type == "paired") {
      ...
      if (!is.null(PairedMarkers) && nrow(PairedMarkers) > 0) {
        ...
        PairedMarkers[, "test_group_number"] <- as.integer(table(PairedMarkers[[
          "gene"
        ]])[PairedMarkers[, "gene"]])
        PairedMarkersMatrix <- as.data.frame.matrix(table(PairedMarkers[, c(
          "gene",
          "group1"
        )]))
    ...
    if (markers_type == "conserved") {
      ...
      ConservedMarkers <- do.call(
        rbind.data.frame,
        lapply(ConservedMarkers, function(x) {
          x[, c(
            "avg_log2FC",
            "pct.1",
            "pct.2",
    ...
    if (any(!sps %in% names(db_list)) || any(!db %in% names(db_list[[sps]]))) {
      ...
      if (is.null(custom_TERM2GENE)) {
        ...
        if (any(db == "CellChat") && (!"CellChat" %in% names(db_list[[sps]]))) {
          ...
          lr[["receptor_list"]] <- lapply(lr_list, function(x) {
            paste0("receptor_", x[2:length(x)])
          })
    ...
    for (term in names(db_list[[sps]])) {
      ...
      if (length(IDtypes) > 0) {
        ...
        if (grepl("MSigDB_", term)) {
          ...
          map <- aggregate(map, by = list(map[[1]]), FUN = function(x) {
            list(unique(x))
          })
          rownames(map) <- map[, 1]
        } else {
...
RunEnrichment <- function(
  ...
  if (isTRUE(db_combine)) {
    ...
    TERM2GENE <- do.call(
      rbind,
      lapply(db_list[[species]], function(x) {
        x[["TERM2GENE"]][, c("Term", IDtype)]
      })
    )
    TERM2NAME <- do.call(
      rbind,
    ...
    version <- unlist(lapply(db_list[[species]], function(x) {
...
RunMonocle2 <- function(
  ...
  cds <- monocle::newCellDataSet(
    ...
    expressionFamily = do.call(
      get(expressionFamily, envir = getNamespace("VGAM")),
      args = list()
    )
  )
...
orderCells <- function(
  ...
  if (cds@dim_reduce_type == "ICA") {
    ...
    mst_branch_nodes <- igraph::V(monocle::minSpanningTree(cds))[which(
      igraph::degree(monocle::minSpanningTree(cds)) > 2
    )]$name
    monocle::minSpanningTree(cds) <- dp_mst
    cds@auxOrderingData[[
      cds@dim_reduce_type
    ...
    cells_mapped_to_graph_root <- igraph::V(monocle::minSpanningTree(cds))[
      cells_mapped_to_graph_root
    ]$name
    tip_leaves <- names(which(
      igraph::degree(monocle::minSpanningTree(cds)) == 1
    ))
...
project2MST <- function(cds, Projection_Method) {
  ...
  if (!is.function(Projection_Method)) {
    ...
    for (i in 1:length(closest_vertex)) {
      neighbors <- names(igraph::V(dp_mst)[suppressWarnings(nei(
...
RunMonocle3 <- function(
  ...
  if (!requireNamespace("monocle3", quietly = TRUE)) {
    # || packageVersion("monocle3") < package_version("1.2.0")
...
#'
#' @return Returns the modified Seurat object with the calculated dynamic features stored in the tools layer.
#'
...
RunDynamicFeatures <- function(
  ...
  for (l in lineages) {
    ...
    if (is.null(features)) {
      ...
      HVF <- VariableFeatures(
        FindVariableFeatures(srt_sub, nfeatures = n_candidates, assay = assay),
        assay = assay
      )
  ...
  for (i in seq_along(lineages)) {
    ...
    system.time({
      gam_out <- bplapply(
        ...
        function(n, Y_ordered, t_ordered, l_libsize, family) {
          ...
          peaktime <- median(t_ordered[
            fitted.values > quantile(fitted.values, 0.99, na.rm = TRUE)
          ])
          valleytime <- median(t_ordered[
    ...
    DynamicFeatures <- as.data.frame(do.call(
      rbind.data.frame,
      lapply(gam_out, function(x) {
...
srt_to_adata <- function(
  ...
  if (length(slot_layers) == 1) {
    ...
    stop(
      "slot_layers must be one character or the same length of the assay_layers"
    )
  }
  ...
  var <- srt[[assay_X]]@meta.data[
    match(features, srt[[assay_X]]@meta.data$var.features),
    ,
    drop = FALSE
  ]
  if (ncol(var) > 0) {
  ...
  X <- t(GetAssayData(srt, assay = assay_X, layer = layer_X)[
    features,
    ,
    drop = FALSE
  ])
...
adata_to_srt <- function(adata) {
  ...
  if (length(keys) > 0) {
    for (k in keys) {
      ...
      srt[[py_to_r_auto(k)]] <- CreateDimReducObject(
        embeddings = obsm,
        assay = "RNA",
...
RunPAGA <- function(
  ...
  SCP_analysis <- reticulate::import_from_path(
    "SCP_analysis",
    path = system.file("python", package = "SCP", mustWork = TRUE),
    convert = TRUE
  )
...
#' Run DE test with future
#'
#' @inheritParams RunDEtest
#' @export
...
RunDEtest2 <- function(
  ...
  p.adjust.method = "bonferroni",
  slot = "data",
  assay = NULL, #BPPARAM = BiocParallel::bpparam(),
  seed = 11,
  verbose = TRUE,
  ...
) {
  ...
  if (slot == "data" && status != "log_normalized_counts") {
    ...
    if (status == "raw_normalized_counts") {
      warning(
        "Data in the 'data' slot is raw_normalized_counts. Perform NormalizeData(LogNormalize) on the data.",
        immediate. = TRUE
      )
      srt <- NormalizeData(
        object = srt,
  ...
  if (!is.null(cells1) || !is.null(group1)) {
    ...
    if (markers_type == "disturbed") {
      ...
      if (!is.null(DisturbedMarkers) && nrow(DisturbedMarkers) > 0) {
        ...
        DisturbedMarkers[,
          "test_group_number"
        ] <- as.integer(table(unique(DisturbedMarkers[,
          c("gene", "group1")
        ])[["gene"]])[DisturbedMarkers[,
          "gene"
        ]])
